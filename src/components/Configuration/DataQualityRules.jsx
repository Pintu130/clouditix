...
import {
  fetchInsertTableData,
  fetchTableData,
  fetchUpdateTableData,
  fetchdatasource,
  fetchvalidationRule,
  tableData,
} from "@/assets/data";
...
const DataQualityRules = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (respo?.data?.length > 0) {
        const response = respo?.data?.filter(
          (item) => formData?.dataSource?.label === item.dataSource
        );
      ...
      if (respo?.data?.length > 0) {
        const response = respo?.data?.filter(
          (item) => formData?.entity?.label === item.tableName
        );
  ...
  const handleSearchData = (data) => {
    if (Object.keys(data).length > 0) {
      const searchData = searchDatas?.filter((item) => {
        if (
          (data?.datasource?.value === "all" ||
            item?.dataSource === data?.datasource?.value) &&
          (data?.entity?.value === "all" ||
            item?.tableName === data?.entity?.value) &&
          (data?.attribute?.value === "all" ||
            item?.columnName === data?.attribute?.value) &&
          (data?.rule?.value === "all" ||
            item?.validationRule === data?.rule?.value) &&
                    ...
                    <div className="flex flex-col w-full items-start lg:max-w-[70%] 2xl:max-w-[80%] gap-1 custom-select">
                      ...
                      <div className="w-full max-w-[300px] lg:max-w-[100%]">
                        <CustomInput
                          ...
                          onChange={(e) =>
                            handleFromData(e.target.value, "ruleParameters")
                          }
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col w-full items-start lg:max-w-[70%] 2xl:max-w-[80%] gap-1 custom-select">
                      <div className="flex items-center w-full gap-2 custom-select ">
                        <input
                          ...
                          onChange={(e) =>
                            handleFromData(e.target.checked, "isMandatory")
                          }
                        />
                        <label
                          htmlFor="speciality"
                    ...
                    <div className="flex flex-col w-full items-start lg:max-w-[70%] 2xl:max-w-[80%] gap-1 custom-select">
                      <div className="flex items-center w-full gap-2 custom-select ">
                        <input
                          ...
                          onChange={(e) =>
                            handleFromData(e.target.checked, "isActive")
                          }
                        />
                        <label
                          htmlFor="speciality"
