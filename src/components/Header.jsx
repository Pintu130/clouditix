...
function Header({ children, handleChangeTab, selectedTab, handlelogout }) {
  ...
  const [selectedPage, setselectedPage] = useState("Guest 360 Dashboard");
  ...
  function MenuItem({ item, handleLinkTo }) {
    ...
    useEffect(() => {
      if (
        item?.children?.map((item) => item?.linkTo).includes(selectedTab) ||
        (item?.children?.length &&
          item?.children[0].linkTo === "intakeManagement" &&
          selectedPage === "queryView")
      ) {
        setIsChildOpen(true);
      } else {
    ...
    const handleSubChildClick = (item) => {
      handleLinkTo(item);
      if (item?.linkTo === "guest-data-create") {
        dispatch(setDetails(0));
      }
    ...
    return (
      <div
        className={`flex  ${item.hasChildren ? "flex-col" : ""} 
      items-${item.hasChildren ? "start" : "center hover:bg-[#EBF2FE]"
          } cursor-pointer  gap-2 py-2 ${selectedPage === item?.linkTo ? "bg-[#EBF2FE] " : "bg-white "
          }
        ${selectedPage === item?.linkTo
            ? !item.hasChildren
              ? "border-l-4 border-[#046e04] "
      ...
      >
        <button
          onClick={
            item.hasChildren
              ? () => toggleChild(item)
              : () => handleSubChildClick(item)
          }
          className="flex items-center w-full pl-3"
        >
          <div
            className={`${selectedPage === item?.linkTo ||
              (item?.linkTo === "medicalRecordsCategories" &&
                selectedPage === "medicalRecordsRules")
              ? "text-[#046e04]"
              : "text-gray-G60"
              }`}
          >
          ...
          <div className="flex justify-between w-full">
            <span
              className={`pl-2 text-base font-OpenSans leading-normal 
            ${selectedPage === item?.linkTo
            ...
            {item.hasChildren &&
              (isChildOpen ? (
                <MdExpandLess className="w-6 h-6 mx-3" />
              ) : (
                <MdExpandMore className="w-6 h-6 mx-3" />
              ))}
          </div>
        </button>
            ...
            {item.children.map((child, index) => {
              return (
                <li
                  ...
                  onClick={
                    !child.hasChildren
                      ? () => handleSubChildClick(child)
                      : undefined
                  }
                  className={`
                pl-8 py-2 hover:bg-[#EBF2FE] font-medium flex items-center gap-2
                  ${selectedPage === child?.linkTo ||
                      (child?.linkTo === "intakeManagement" &&
                        selectedPage === "queryView")
                      ? !child.hasChildren
                        ? "border-l-4 text-[#046e04] border-[#046e04]"
                        : "border-l-4 border-transparent"
                      : "border-l-4 border-transparent"
                    }
                ...
                >
                  <div
                    className={`${selectedPage === child?.linkTo
                      ? "text-[#046e04]"
                      : "text-gray-G60 "
                      }`}
                  >
                    ...
                    <ul className="w-full py-2 pl-8">
                      {child.children.map((subChild, subIndex) => (
                        <li
                          ...
                          className="py-2 hover:bg-[#EBF2FE]"
                        >
                          {subChild.text}
                        </li>
                      ))}
    ...
    >
      <div className="px-3 pt-3 shadow-[0px_4px_6px_0px_#1143911A]  z-10 ">
        <div className="flex justify-between border-b ">
          <button className="flex items-center flex-shrink-0 gap-3 ">
            {!isSidebarOpen ? (
              <AiOutlineMenu
                className="w-4 h-4 cursor-pointer sm:h-6 sm:w-6"
          ...
          <div className="flex gap-4 items-center ">
            ...
            />
            <div
              className="relative flex-shrink-0 bg-[#046e04] rounded-full w-9 h-9"
              onClick={(e) => handleUserIconClick(e)}
            >
              <div className="absolute flex items-center justify-center w-full h-full text-base font-bold leading-normal text-white cursor-pointer">
                CR
              </div>
      ...
      <div className="relative flex w-full h-full md:static">
        {isSidebarOpen && (
          ...
          >
            <div className="text-black absolute -top-[7px] bg-transparent h-5 border-r border-[#A6A6A6] w-full"></div>
        ...
        <div
          className={`flex transition-all duration-700 items-start justify-start w-full`}
        >
          {children}
        </div>
      </div>
